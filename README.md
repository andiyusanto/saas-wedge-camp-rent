...
- Riwayat Transaksi (owner-only) — rekap semua transaksi termasuk yang sudah Selesai/Dibatalkan (Jaminan & Denda cuma menampilkan status aktif, jadi transaksi yang sudah ditutup sebelumnya tidak terlihat di mana pun lagi). Kartu ringkasan (Total Transaksi, Sudah Diterima, Piutang, Jatuh Tempo Hari Ini), filter tanggal/status/pencarian, dan ekspor CSV. Query langsung ke Supabase dari frontend (bukan lewat backend) karena tidak butuh perhitungan live seperti Jaminan & Denda — data historis sudah final.
