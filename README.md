...
- Katalog & stok alat — kode alat, kategori, varian/ukuran/warna opsional (pilih dari nilai yang sudah pernah dipakai di katalog atau ketik baru, pola sama seperti Bilbo-Outdoors), foto (unggah langsung dari galeri/kamera, otomatis dikompres & disimpan ke Cloudflare R2 — bukan tempel URL manual), deskripsi, catatan kondisi, jumlah unit, harga sewa/hari, harga bertingkat opsional (tarif lebih murah per hari setelah sewa melewati N hari), jeda persiapan opsional dalam hari (migration 026 — berapa hari alat butuh diangin-anginkan/dicek sebelum siap disewa lagi setelah kembali; diterapkan simetris di `usedUnitsOn()`, mundur sebelum `start_date` booking lain DAN maju setelah `end_date`-nya, konsisten di Kalender, validasi kapasitas `POST /bookings`, maupun pencarian ketersediaan Etalase Online — lihat CLAUDE.md bagian 6); bisa diedit atau dinonaktifkan (soft-delete, histori booking lama tetap utuh)
