...
- Katalog & stok alat — kode alat, kategori, varian/ukuran/warna opsional (pilih dari nilai yang sudah pernah dipakai di katalog atau ketik baru, pola sama seperti Bilbo-Outdoors), foto (unggah langsung dari galeri/kamera, otomatis dikompres & disimpan ke Cloudflare R2 — bukan tempel URL manual), deskripsi, catatan kondisi, jumlah unit, harga sewa/hari, harga bertingkat opsional (tarif lebih murah per hari setelah sewa melewati N hari), jeda persiapan opsional dalam **jam** (migration 026, direvisi ke jam migration 028 — berapa jam alat butuh dibersihkan/dicek sebelum siap disewa lagi setelah kembali; 0 = tersedia lagi hari yang sama; dibulatkan ke atas per hari kalender saat dipakai — bukan presisi sub-hari, lihat CLAUDE.md bagian 6 untuk detail; jendela untuk booking yang sudah selesai/telat dijangkarkan ke hari SUNGGUHAN barang kembali `actual_return_at`, bukan jadwal `end_date`; konsisten di Kalender, validasi kapasitas `POST /bookings`, maupun pencarian ketersediaan Etalase Online); bisa diedit atau dinonaktifkan (soft-delete, histori booking lama tetap utuh)
- Kelola Usaha (owner-only, satu tab gabungan) — bagian **Info Usaha**: edit nama usaha, nama pemilik, no. telepon, alamat usaha (migration 025 — `businesses.address`, sebelumnya sudah didesain di tipe `ReceiptData.businessAddress` tapi tidak pernah punya sumber data; sekarang aktif dan ikut tercetak di struk WhatsApp & Nota Sewa cetak/PDF), dan toleransi telat (jam) kapan saja (migration 018 membatasi RLS `businesses` UPDATE ke owner, sebelumnya semua anggota bisa update). Bagian **Kelola Karyawan**: undang staf lewat link sekali-pakai (kedaluwarsa 7 hari), tanpa perlu pemilik membuatkan username/password. Peran (`karyawan`/`owner`) murni label atribusi untuk fitur operasional (kalender, transaksi, transaksi aktif, katalog) — akses di situ sama untuk semua anggota. Satu pengecualian: tab Kelola Usaha ini sendiri (Info Usaha maupun undang/keluarkan anggota) cuma bisa diakses pemilik, ditegakkan di RLS (migration 014, 018) dan backend, bukan cuma disembunyikan di UI. Layar login yang dibuka lewat link undangan (`?invite=...`) menampilkan nama usaha & peran yang mengundang **sebelum** login (`GET /api/team/invites/preview`, migration 016 — function `SECURITY DEFINER` yang dibatasi ketat, cuma untuk kode undangan yang valid).
- Riwayat Transaksi (owner-only) — rekap semua transaksi termasuk yang sudah Selesai/Dibatalkan (Transaksi Aktif cuma menampilkan status aktif, jadi transaksi yang sudah ditutup sebelumnya tidak terlihat di mana pun lagi). Kartu ringkasan (Total Transaksi, Sudah Diterima, Piutang, Jatuh Tempo Hari Ini — tiga pertama sengaja tidak menghitung transaksi Dibatalkan), filter tanggal/status/pencarian, ekspor CSV, dan bagian Peralatan Terlaris Disewa (dikelompokkan per kategori, collapsible, diurutkan dari jumlah unit tersewa terbanyak). Query langsung ke Supabase dari frontend (bukan lewat backend) karena tidak butuh perhitungan live seperti Transaksi Aktif — data historis sudah final.
- Etalase Online (owner-only untuk pengaturannya, halamannya sendiri publik) — **fitur sungguhan, sudah bisa dipakai**, bukan fake-door. Toggle aktifkan + slug (auto dari nama usaha, bisa diedit, unik — dicek ketersediaannya live sambil ketik lewat RPC `is_slug_available()`, migration 022, debounced 300ms; penjaga sebenarnya tetap unique constraint di DB, ini cuma feedback lebih awal) menghasilkan halaman publik di backend — `toko.sewalog.com/{slug}` kalau custom domain-nya sudah disambungkan (lihat Deploy langkah 8), fallback ke `/toko/{slug}` di URL backend biasa kalau belum — TANPA LOGIN, etalase katalog alat + kontak (nama usaha, kota, deskripsi, alamat, jam operasional — textarea multi-baris, bebas format per hari kalau jamnya beda-beda, baris baru ikut tampil di halaman publik lewat `white-space:pre-line` — tombol WhatsApp) — alamat & nomor WA publik di sini otomatis pre-fill dari Info Usaha (`businesses.address`/`phone`) saat pertama diaktifkan, tapi sesudah disimpan independen sepenuhnya, boleh beda dari Info Usaha kapan saja — filter kategori (pill di atas katalog, cuma muncul kalau alatnya punya lebih dari satu kategori — murni show/hide sisi klien, tidak fetch ulang) dan foto alat yang bisa di-zoom (lightbox sederhana, vanilla JS, mirip pola `CategoryFilterTabs`/`ImagePreviewModal` di Bilbo-Outdoors tapi tanpa cart/quantity-selector-nya — itu bagian storefront+checkout Bilbo yang eksplisit bukan yang dibangun di sini) plus **pencarian ketersediaan per rentang tanggal** (dua date-picker, tanggal ambil & kembali — bukan cuma satu hari, karena rental berlangsung beberapa hari dan yang relevan adalah cukup-tidaknya stok di SEMUA hari dalam periode itu; angka yang ditampilkan adalah titik terendah/terburuk sepanjang rentang, sama seperti validasi kapasitas `POST /bookings`; menampilkan jumlah unit tersisa persis per alat — keputusan sadar vendor untuk exposure paling terbuka, lihat CLAUDE.md bagian 6), **bukan toko online/checkout** (tidak ada keranjang/pemesanan/pembayaran — transaksi tetap manual lewat WA). Data di tabel `public_pages` (migration 019), terpisah dari `business_profiles`/Rimbasewa (lihat CLAUDE.md bagian 6 & 7). Halaman publiknya di-render server-side sebagai HTML mentah oleh `backend/src/routes/etalase.ts` (bukan route React) lewat function database `get_public_page()`/`get_public_availability()` (`SECURITY DEFINER`, kolom aman hardcode per function) — supaya Open Graph tags & JSON-LD-nya benar tanpa perlu JS jalan dulu (penting karena kanal distribusi utamanya link WhatsApp), sementara pencarian ketersediaannya sendiri progressive-enhancement lewat fetch JS terpisah (`GET /api/public/etalase/{slug}/availability?start_date=...&end_date=...`, migration 021) yang tidak mengganggu HTML awal itu.
- Toko Online (owner-only) — **fake-door, bukan fitur sungguhan**. Halaman penjelasan rencana toko online berbayar per-vendor + tombol "Tertarik? Kasih Tahu Kami" ke WhatsApp, dipakai buat lihat sinyal minat vendor pilot secara murah sebelum commit ke pembangunan aslinya (lihat non-goals di `CLAUDE.md` bagian 8 dan `research/online-checkout-payment-roadmap-2026-08.md`). Beda dari Etalase Online di atas: yang ini soal transaksi/pembayaran online sungguhan, bukan sekadar halaman info.
- Panduan penggunaan — halaman referensi in-app yang menjelaskan tiap fitur dari sisi vendor (tab **Panduan** di navbar).
- Audit trail — setiap perubahan status transaksi (aktif/telat/selesai/dibatalkan) tercatat siapa & kapan (`booking_status_history`, append-only) dan bisa dilihat langsung lewat "Riwayat Status" di tiap kartu transaksi pada layar Transaksi Aktif; perubahan data alat (tambah/edit/nonaktifkan) tercatat otomatis lewat trigger database (`items.created_by`/`updated_by`/`updated_at`/`deactivated_by`) dan ditampilkan sebagai baris "Diperbarui oleh..." di tiap kartu Katalog & Stok Alat.
...
```
frontend/       React SPA (Vite) — app produk (login, kalender, transaksi, transaksi aktif)
...
- `PUBLIC_BASE_URL` = URL backend ini sendiri setelah deploy (contoh: `https://sewalog-backend.onrender.com`) — fallback OG tags halaman Etalase Online (`/toko/{slug}`) kalau `TOKO_PUBLIC_HOST` di bawah belum diisi
- `TOKO_PUBLIC_HOST` = *(opsional, isi belakangan — lihat langkah 8)* domain khusus Etalase Online (mis. `toko.sewalog.com`), TANPA `https://` dan TANPA path
...
### 8. (Opsional) Sambungkan domain toko.sewalog.com untuk Etalase Online

Tanpa langkah ini, halaman publik Etalase Online tetap bisa diakses lewat `https://<url-backend-render-mu>/toko/{slug}` — langkah ini murni supaya link yang dibagikan vendor ke pelanggannya lebih rapi (`toko.sewalog.com/{slug}`, bukan URL backend mentah), sama seperti custom domain `image.sewalog.com` yang sudah disambungkan ke bucket R2.

1. Render Dashboard → service **backend** → **Settings → Custom Domains → Add Custom Domain** → masukkan `toko.sewalog.com`.
2. Render akan kasih target CNAME — tambahkan sebagai DNS record di provider domainmu (bukan bikin CNAME manual sendiri di luar alur ini, sama seperti catatan R2 di atas soal error 1014 Cloudflare kalau setup manual).
3. Tunggu status domainnya jadi **Verified** di Render.
4. Set environment variable di service backend: `TOKO_PUBLIC_HOST=toko.sewalog.com` (tanpa `https://`, tanpa trailing slash) → simpan (redeploy otomatis).
5. Set environment variable di service **frontend**: `VITE_TOKO_PUBLIC_BASE_URL=https://toko.sewalog.com` → build ulang (Vite meng-inline env var saat build, jadi perlu manual redeploy/trigger build kalau diisi setelah deploy pertama).

Backend yang sama (satu Web Service, satu proses) tetap menjawab baik `api`-style URL asli maupun `toko.sewalog.com` — dibedakan lewat header `Host`, bukan deploy terpisah (lihat komentar `TOKO_PUBLIC_HOST` di `backend/src/routes/etalase.ts`).

### Catatan soal free tier

- **Static Site** (frontend, landing) tidak pernah "tidur" — selalu langsung responsif.
- **Web Service** (backend) di free tier akan *spin down* setelah idle beberapa saat, dan butuh waktu untuk "bangun" lagi (cold start) di request pertama setelahnya — bisa terasa lambat beberapa puluh detik. Kalau mau demo langsung ke vendor, buka dulu halaman Kalender/Catat Transaksi/Transaksi Aktif beberapa menit sebelum sesi demo dimulai supaya backend sudah "bangun".
- Login, onboarding, dan sebagian besar Kelola Alat memanggil Supabase langsung dari frontend (tidak lewat backend), jadi tidak kena cold start ini. Tiga fitur inti MVP, upload foto alat (harus lewat backend karena kredensial R2), dan Kelola Usaha (undangan karyawan) tetap lewat backend, jadi bisa terasa lambat di request pertama setelah idle.
